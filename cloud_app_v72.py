...
APP_VERSION = "v7.2-us-hk-quote-fallback-tjgc-us"
...
st.sidebar.info("美股、港股若未填股數/成本，會以觀察清單方式抓報價與漲跌；不納入市值。TJGC 請設為 US。")
...
def show_diag():
    ...
    st.info("若港股或美股沒有報價，請確認 Yahoo Finance 代號。例如比亞迪為 1211.HK；TME(ADR) 已自動轉成 TME。TJGC 是美股 Nasdaq 代號，請設為市場 US；港股需使用 Yahoo 格式如 1211.HK。若股數為 0，只會顯示報價/漲跌，不會有市值與損益。")
