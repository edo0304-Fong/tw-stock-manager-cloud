...
def parse_paste_text(text: str) -> pd.DataFrame:
    ...
    for line in str(text or "").splitlines():
        ...
        market_hint = parts[4] if len(parts) >= 5 else ""
        ...
        rows.append({
            ...
            "currency": currency_for_market(market), "shares": shares, "avg_cost": avg_cost, "last_price": 0,
            "note": "貼上匯入", "updated_at": now_text()
        })
    return normalize_portfolio_df(pd.DataFrame(rows))
...
def fetch_yahoo_quotes(yahoo_symbols: tuple[str, ...]) -> pd.DataFrame:
    ...
    for i in range(0, len(symbols), 60):
        ...
        try:
            resp = requests.get("https://query1.finance.yahoo.com/v7/finance/quote", params={"symbols": ",".join(batch), "lang": "zh-TW", "region": "TW"}, headers=headers, timeout=10)
        ...
        for it in data:
            ...
            rows.append({"yahoo_symbol": ys, "current_price": safe_float(price), "previous_close": safe_float(prev), "change": safe_float(change), "change_pct": safe_float(change_pct), "quote_source": "YahooQuote"})
        for ys in set(batch) - got:
            rows.append({"yahoo_symbol": ys, "current_price": 0, "previous_close": 0, "change": 0, "change_pct": 0, "quote_source": "NoData"})
        time.sleep(0.05)
...
# Sidebar
# -------------------------
PAGE_OPTIONS = ["總覽", "匯入/更新持股", "台股個股", "台股ETF", "美股", "港股", "全球統計", "報價診斷"]
...
st.sidebar.info("美股、港股若未填股數/成本，會以觀察清單方式統計報價與漲跌；不納入市值。")
...
def show_import():
    st.header("匯入 / 更新持股")
    st.caption("支援你上傳的 IFong 持股 Excel：台股個股、台股ETF、美股、港股四張表。也支援手機貼上簡易格式。")
    ...
    default_text = """2303.TW 聯電 4000 5.27
5289.TWO 宜鼎 500 1774.6
NVDA NVIDIA 0 0 US
1211.HK 比亞迪 0 0"""
    txt = st.text_area("每行：代號 名稱 股數 成本 [市場]；美股/港股可只貼代號作觀察清單", value="", placeholder=default_text, height=160)
...
def show_diag():
    ...
    st.info("若港股或美股沒有報價，請確認 Yahoo Finance 代號。例如比亞迪為 1211.HK；TME(ADR) 已自動轉成 TME。TJGC 若非 Yahoo 可辨識代號，需改成實際上市代號。")
